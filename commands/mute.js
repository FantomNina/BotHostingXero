...
module.exports = {
    ...
    async execute(client, message, cmd, args, Discord){
        ...
        if (member.roles.cache.find(r => r.name.toLowerCase() === 'muted')) return message.rpermisseply('This USER is already muted!')
        if (!message.guild.me.hasPermission('MANAGE_ROLES')) return message.reply('I do not have the ions to mute the user! `MANAGE_ROLES`')
        if (member.id === message.author.id) return message.reply('You cant mute yourself lol! ')
        if (member.id === client.user.id) return message.reply('You cant mute me hahahahah!')
